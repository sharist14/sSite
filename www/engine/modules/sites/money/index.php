...
class index extends parentTemplate
{
    ...
    function _act_(){
        ...
        // ЗАРПЛАТНЫЕ СЧЕТА
        $total_money = 0;
        $accounts_salary = [1,2];   // 1 - наличные, 2 - Карта СПБ(зарплатная)
        $sql = 'SELECT `money_accounts`.*,`money_accounts`.`title` as `acc_title`, `currency`.`sign_rus`, SUM(`money_flow`.`s`) as `summ_acc` FROM `money_accounts` 
                ...
                WHERE `money_accounts`.`id` IN('.implode(',', $accounts_salary).') 
        ...
        while($row = $moneyAccObj->fetch_assoc()){
            ...
            $tt = $tpl_acc_row;
            $tt = set($tt, 'title', $row['acc_title']);

            // Сумма на счету
            $summ_acc = is_numeric($row['summ_acc'])? floatval($row['summ_acc']) : 0;
            $tt = set($tt, 'summ_account', $summ_acc);

            $tpl = setm($tpl, 'salary_account_rows', $tt);

            $total_money += $summ_acc;
        }
        ...
        // ОСТАЛЬНЫЕ СЧЕТА
//        $accounts_other = [3];   // 3 - общие сбережения на отдых
//        $sql = 'SELECT `money_accounts`.*,`money_accounts`.`title` as `acc_title`, `currency`.`sign_rus`, SUM(`money_flow`.`s`) as `summ_acc` FROM `money_accounts`
//                LEFT JOIN `money_flow` ON(`money_accounts`.`id` = `money_flow`.`money_account_id`)
//                LEFT JOIN `currency` ON(`money_accounts`.`currency_id` = `currency`.`id`)
//                WHERE `money_accounts`.`id` IN('.implode(',', $accounts_other).')
//                GROUP BY `money_accounts`.`id`
//                ORDER BY `order` DESC
//        ';
//        $moneyAccObj = $table_acc->_DB_->q($sql);
