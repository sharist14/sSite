...
class _moneyFlow_abstract extends parentTemplate {
     ...
     */
    function _act_(){
        $tpl = get_template('', '_moneyFlow_abstract', 'body');
        $tpldr = get_template('', '_moneyFlow_abstract', 'day_row');
        $tplpr = get_template('', '_moneyFlow_abstract', 'flow_row');

        $tpl = set($tpl, 'type_ru', $this->type_ru );
        $tpl = set($tpl, 'type_en', $this->type_en );
        $tpl = set($tpl, 'type_ru_title', ucfirst_utf8($this->type_ru) );

        $accounts_salary = [1, 2];   // Зарплатные счета 1 - наличные, 2 - Карта СПБ(зарплатная)
        ...
        // Выводим инфу со статусами при редиректе на текущую страницу
        if($this->params['frame_info']){
            $tpl = set($tpl, 'id', $this->params['id']);
            $tpl = set($tpl, 'page', $this->params['page']);
            $tpl = set($tpl, 'frame_info', $this->params['frame_info']);
        }
        ...
        if($_GET['acc1'] && $_GET['acc2'] && $this->type_en == 'transfer'){
            ...
            $db = $this->table->_DB_->q($this->table->getSelectF('FROM_UNIXTIME(`t`, "%m.%Y") AS `data_short`', $compare . ' GROUP BY `data_short` ORDER BY `t` DESC'));
            while ($row = $db->fetch_assoc()) {
                $selected = '';
                if ($row['data_short'] == $page_str) {
                    $selected = 'selected';
                }

                $tpl = setm($tpl, 'page_options', '<option value="' . $row['data_short'] . '" ' . $selected . '>' . $row['data_short'] . '</option>');
            }
            ...
            if($this->str_month_summ){
                ...
                $tpl = setm($tpl, 'month_sum', $month_sum);
            } else{
                $tpl = set($tpl, 'hide_month_summ', 'hide' );
            }
        ...
        while($row = $db->fetch_assoc()){
            ...
            while($row2 = $db2->fetch_assoc() ){
                ...
                $download_receipt = '<a href="/money_'.$this->type_en.'?act=view_receipt&id='.$row2["id"].'" class="btn p-0 d-inline-block"><img style="width: 40px" src="'._IMG_.'/_static/receipt_icon.png" alt=""></a>';
                ...
                if($row2['linked_id']){
                    ...
                    $add_info = ' "'.$title_acc_from.'" -> "'.$title_acc_to.'"';
                }
                $tt2 = set($tt2, 'type_title', $type_title . $add_info);

                // Лого юр лица
                $entity_logo = 'no_entity.png';
                if($row2['entity_id']){
                    $entityObj->byId($row2['entity_id']);
                    if($entityObj->img){
                        $entity_logo = $entityObj->img;
                    }
                } elseif($json = json_decode($row2['json_data'], true)){
                    $inn = trim($json['document']['receipt']['userInn']);
                    $db3 = $entityObj->_DB_->q($entityObj->getSelect(' AND `inn` = "'.$inn.'" AND `img` != ""'));
                    if($row3 = $db3->fetch_assoc()){
                        $entity_logo = $row3['img'];
                    }
                }
     ...
     */
    function _act_edit(){
        $tpl = get_template('', '_moneyFlow_abstract', 'add_form');
        ...
        if($_GET['id']){
            ...
            while($row = $flowObj->fetch_assoc()){
                if($row['s'] < 0){
                    ...
                    $tpl = set($tpl, 'disabled_debt', 'disabled');
        ...
        if($this->block_account_from){
            ...
            while($row = $moneyAccObj->fetch_assoc()) {
                ...
                // Считаем сколько денег на счете
        ...
        if($this->block_account_to){
            ...
            while($row = $moneyAccObj->fetch_assoc()) {
                ...
                $tpl = setm($tpl, 'account_options_to', '<option value="'.$row['id'].'" '.$selected.'>'.$row['title'].'</option>');
            }
        } else{
        ...
        if($this->block_type){
            ...
            $cat_obj = $table->_DB_->q($table->getSelect($filter_type));
        ...
        if($this->block_entity){
            $entityObj = new table('entity');
            $entity_arr = $entityObj->getTitles();
            foreach($entity_arr as $k => $v){
                $selected = $this->entity_id == $k ? 'selected' : '';
                $tpl = setm($tpl, 'entity_options', '<option value="'.$k.'"'.$selected.'>'.$v.'</option>');
            }
        } else{
    ...
    function _act_delete_flow($id = '', $redirect = ''){
        ...
        _redirect('/'.$redirect.'?frame_info=delete');
    }
    ...
    function do_save($data, $sign = 1){
        ...
        if($result['status'] != 'duplicate') {
            ...
            // Если сняли галку с подтвержденного вручную, то помечаем операцию как новую
            if($flowObj->status_id == 5 && $data['manual_confirm'] == 0){
                $flowObj->set('status_id', 1);
            }

            // todo Проверять, если сняли галку взять в долг, нужно будет удалить операции по долгу
