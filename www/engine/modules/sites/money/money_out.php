...
class money_out extends _moneyFlow_abstract
{
    var $table;
    ...
    function _act_save() {
        ...
        if($result['status'] == 'create' && $data['fn'] && $data['i'] && $data['fp'] ){
            $this->_act_get_receipt_from_nalog($result['id'], true);
        }
        ...
        _redirect('/money_out/?frame_info='.$result['status'].$page.'&id='.$result['id']);
    }
     ...
     */
    function _act_get_receipt_from_nalog($id = '', $return_result = false){
        ...
        if( !$flowObj->ticket_id ){
            ...
            if($api_data['status'] == 2 && $api_data['id']) {
                ...
                $flowObj->set('status_id', 4);   // статус: данные некорректны
                $flowObj->save();

                $result['error'] = 'При попытке получить от api ticket_id (для flow_id '.$flow_id.') пришла ошибка, что указаны некорректные данные в зaпросе';
                add_log('Налоговая:' . $result['error'], 'nalog.log', 2);
        ...
        if( $flowObj->ticket_id && !$flowObj->json_data ) {
            ...
            if($receipt_data['status'] == 2 && $receipt_data['ticket']) {
                ...
                $result['error'] = 'При попытке получить от api json с данными по чеку (для flow_id '.$flow_id.') пришла ошибка';
                add_log('Налоговая:' . $result['error'], 'nalog.log', 2);
        ...
        if(!$return_result){
            _redirect('/money_out/?frame_info='.$status.$page.'&id='.$flowObj->id);
        } else{
            return true;
        }
